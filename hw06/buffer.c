...
int main(){
    ...
    char * b_contents = bufferContents(b);
    printf("Buffer contents: %s\n", b_contents);
    printf("Buffer size length: %zu\n", bufferSize(b));
    printf("Buffer cursors number: %zu\n", bufferCursors(b));
    free(b_contents);
    ...
    bufferInsert(b, 2, 'c');
    bufferInsert(b, 2, 'e');
    char * second_b_contents = bufferContents(b);
    printf("Buffer contents: %s\n", second_b_contents);
    free(second_b_contents);
    printf("Buffer size length: %zu\n", bufferSize(b));
    printf("Buffer cursors number: %zu\n", bufferCursors(b));
    for (size_t i = 0; i < n; i++){
        printf("%zu: %c\n", i, b->cursors[i]->value);
    ...
    printf("------------\n");
    bufferCursorBack(b, 1);
    for (size_t i = 0; i < n; i++){
        printf("%zu: %c\n", i, b->cursors[i]->value);
    }
    bufferInsert(b, 1, 'b');
    bufferCursorForward(b, 1);
    for (size_t i = 0; i < n; i++){
        printf("%zu: %c\n", i, b->cursors[i]->value);
    }
    bufferInsert(b, 1, 'd');
    ...
    char * third_b_contents = bufferContents(b);
    printf("Buffer contents: %s\n", third_b_contents);
    free(third_b_contents);
    printf("Buffer size length: %zu\n", bufferSize(b));
    printf("Buffer cursors number: %zu\n", bufferCursors(b));
    bufferDestroy(b);
}
